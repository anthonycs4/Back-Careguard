import { IsArray, IsBoolean, IsIn, IsOptional, IsString, ValidateNested, MaxLength, Length } from 'class-validator';
...
export class AnimalDto {
  ...
  @IsIn(['PEQUENIO','MEDIANO','GRANDE']) tamanio: 'PEQUENIO'|'MEDIANO'|'GRANDE';
  @IsIn(['AGRESIVO','TRANQUILO','INQUIETO']) personalidad: 'AGRESIVO'|'TRANQUILO'|'INQUIETO';
...
export class CreateMascotasPayloadDto {
  ...
  @IsIn(['EN_HOGAR_MASCOTA','EN_HOGAR_CUIDADOR']) modalidad: 'EN_HOGAR_MASCOTA'|'EN_HOGAR_CUIDADOR';

  @ValidateNested() @Type(() => ContactoMascotaDto)
  contacto: ContactoMascotaDto;

  @IsArray() @ValidateNested({ each: true }) @Type(() => AnimalDto)
