import { IsArray, IsBoolean, IsDateString, IsIn, IsOptional, IsString, ValidateNested, MaxLength, Length } from 'class-validator';
...
export class PersonaNinioDto {
  ...
  @IsIn(['MASCULINO','FEMENINO','NO_BINARIO','PREFIERO_NO_DECIR','DESCONOCIDO']) genero: string;
  ...
  @IsIn(['LECHE_MATERNA','FORMULA','MIXTO','SOLIDOS']) alimentacion: 'LECHE_MATERNA'|'FORMULA'|'MIXTO'|'SOLIDOS';
  ...
  @IsArray() @ValidateNested({ each: true }) @Type(() => MedicamentoDto)
  medicamentos: MedicamentoDto[];
}

export class CreateNiniosPayloadDto {
  @IsIn(['INFANTES','BEBES','ESPECIALES']) servicio: 'INFANTES'|'BEBES'|'ESPECIALES';

  @ValidateNested() @Type(() => ContactoTutorDto)
  contacto_tutor: ContactoTutorDto;

  @IsArray() @ValidateNested({ each: true }) @Type(() => PersonaNinioDto)
