import {
  BadRequestException, Body, Controller, Delete, Get, Param, Patch, Post, Query, Req, UseGuards,UsePipes,
  ValidationPipe,
} from '@nestjs/common';
...
async createAbuelos(
  @Req() req,
  @Body() body: { base: CreateSolicitudBaseDto; payload: CreateAbuelosPayloadDto }
) {
  ...
  if (!payload?.personas?.length || payload.personas.length > 3) {
    throw new BadRequestException('Debes registrar entre 1 y 3 personas a cuidar');
  }
  ...
  const resBase = await fetch(`${this.baseUrl}/rest/v1/solicitudes`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=representation,tx=commit' },
    body: JSON.stringify({
  ...
  if (base.fechas?.length) {
    const fechasPayload = base.fechas.map(f => ({
      solicitud_id: solicitud.id, fecha: f.fecha, hora_inicio: f.hora_inicio, hora_fin: f.hora_fin
    }));
    const resFechas = await fetch(`${this.baseUrl}/rest/v1/solicitud_fechas`, {
      method: 'POST',
  ...
  // 3) Detalle (servicio)
  const resDet = await fetch(`${this.baseUrl}/rest/v1/solicitud_abuelos_detalle`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=minimal' },
    body: JSON.stringify({ solicitud_id: solicitud.id, servicio: payload.servicio }),
  });
  if (!resDet.ok) throw new BadRequestException(await resDet.text());

  // 4) Contacto cercano
  const resCc = await fetch(`${this.baseUrl}/rest/v1/solicitud_contacto_cercano`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=minimal' },
    body: JSON.stringify({
  ...
  for (const p of payload.personas) {
    const resP = await fetch(`${this.baseUrl}/rest/v1/solicitud_abuelos_personas`, {
      method: 'POST',
    ...
    if (p.medicamentos?.length) {
      const medsPayload = p.medicamentos.map(m => ({
        persona_id: persona.id, nombre: m.nombre, frecuencia: m.frecuencia, dosis: m.dosis
      }));
      const resM = await fetch(`${this.baseUrl}/rest/v1/solicitud_abuelos_medicamentos`, {
        method: 'POST',
        headers: { ...this.headersJson(), Prefer: 'return=minimal' },
...
async createNinios(
  @Req() req,
  @Body() body: { base: CreateSolicitudBaseDto; payload: CreateNiniosPayloadDto }
) {
  const usuarioId = (req.userId ?? req.user?.id) as string;
  if (!usuarioId) throw new BadRequestException('Token inválido');
  ...
  const resDet = await fetch(`${this.baseUrl}/rest/v1/solicitud_ninios_detalle`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=minimal' },
    body: JSON.stringify({ solicitud_id: solicitud.id, servicio: payload.servicio }),
  });
  ...
  // 4) Contacto tutor
  const resTutor = await fetch(`${this.baseUrl}/rest/v1/solicitud_contacto_cercano`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=minimal' },
    body: JSON.stringify({
      solicitud_id: solicitud.id,
  ...
  for (const p of payload.personas) {
    const resP = await fetch(`${this.baseUrl}/rest/v1/solicitud_ninios_personas`, {
      method: 'POST',
      headers: { ...this.headersJson(), Prefer: 'return=representation' },
      body: JSON.stringify({
        solicitud_id: solicitud.id,
    ...
    if (p.medicamentos?.length) {
      ...
      const resM = await fetch(`${this.baseUrl}/rest/v1/solicitud_ninios_medicamentos`, {
        method: 'POST',
        headers: { ...this.headersJson(), Prefer: 'return=minimal' },
        body: JSON.stringify(medsPayload),
      });
...
async createMascotas(
  @Req() req,
  @Body() body: { base: CreateSolicitudBaseDto; payload: CreateMascotasPayloadDto }
) {
  const usuarioId = (req.userId ?? req.user?.id) as string;
  if (!usuarioId) throw new BadRequestException('Token inválido');
  ...
  const resDet = await fetch(`${this.baseUrl}/rest/v1/solicitud_mascotas_detalle`, {
    method: 'POST',
    headers: { ...this.headersJson(), Prefer: 'return=minimal' },
    body: JSON.stringify({
      solicitud_id: solicitud.id,
  ...
  for (const a of payload.animales) {
    const resA = await fetch(`${this.baseUrl}/rest/v1/solicitud_mascotas_animales`, {
      method: 'POST',
      headers: { ...this.headersJson(), Prefer: 'return=minimal' },
      body: JSON.stringify({
        solicitud_id: solicitud.id,
  ...
  @Get()
  async listMine(@Req() req, @Query('tipo') tipo?: 'ABUELOS'|'NINIOS'|'MASCOTAS') {
    const usuarioId = (req.userId ?? req.user?.id) as string;
    ...
    const r = await fetch(url, { headers: { apikey: this.serviceKey, Authorization: `Bearer ${this.serviceKey}` }});
...
async abiertas(@Req() req, @Query() q: GetAbiertasQueryDto) {
  ...
  if (!cuidador) throw new BadRequestException('Aún no tienes perfil de cuidador');
  const tipos: string[] = Array.isArray(cuidador.tipos_servicio) ? cuidador.tipos_servicio : [];
  if (tipos.length !== 2) throw new BadRequestException('Perfil de cuidador inválido (tipos_servicio)');
...
async mias(@Req() req, @Query() q: GetMiasQueryDto) {
  ...
  const headers = { apikey: this.serviceKey, Authorization: `Bearer ${this.serviceKey}` };
  ...
  const select = [
    'id','tipo','titulo','descripcion','estado','precio_sugerido','creado_en',
    'solicitud_fechas(fecha,hora_inicio,hora_fin)',
  ...
  const items = rows.map((s: any) => ({
    ...
    postulaciones_count: Array.isArray(s.postulaciones) && s.postulaciones[0]?.count != null
