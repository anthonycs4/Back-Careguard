import { BadRequestException, Body, Controller, Post,Get, Req, UseGuards, UsePipes, Param, ValidationPipe } from '@nestjs/common';
...
dbgPost(@Req() req) {
  ...
  private get serviceKey() { return this.cfg.get<string>('SUPABASE_SERVICE_ROLE_KEY')!; }
  private headersJson() {
...
async crear(@Req() req, @Body() dto: CreatePostulacionDto) {
  ...
  if (!dto.solicitud_id) throw new BadRequestException('solicitud_id es requerido');
  ...
  if (!r.ok) {
    if (txt.includes('duplicate key') || txt.includes('postulaciones_unicas_activas')) {
      throw new BadRequestException('Ya te postulaste a esta solicitud (o está seleccionada).');
    }
  ...
  if (txt) {
    try {
      ...
      throw new BadRequestException(`Respuesta inesperada de Supabase: ${txt}`);
    }
  }
  ...
  if (!row) {
    ...
    return { message: 'Postulación creada', detalle: 'Insert OK (sin cuerpo)' };
  }
...
async getBySolicitud(@Req() req, @Param('id') solicitudId: string) {
  const userId = (req.userId ?? req.user?.id) as string;
  if (!userId) throw new BadRequestException('Token inválido');
  if (!solicitudId) throw new BadRequestException('ID de solicitud requerido');
  ...
  if (solicitud.usuario_id !== userId) throw new BadRequestException('No puedes ver postulaciones de otra solicitud');
...
async aceptar(
  ...
  const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_seleccionar_postulacion`, {
    method: 'POST',
