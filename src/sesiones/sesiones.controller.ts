...
export class SesionesController {
  ...
  private get baseUrl()  { return this.cfg.get<string>('SUPABASE_URL')!; }
  private get svcKey()   { return this.cfg.get<string>('SUPABASE_SERVICE_ROLE_KEY')!; }
  private headersJson()  {
    return { apikey: this.svcKey, Authorization: `Bearer ${this.svcKey}`, 'Content-Type': 'application/json' };
  }
  private headers() {
  ...
  async crearDesdeAsignacion(@Req() req, @Body() dto: CrearDesdeAsignacionDto) {
    ...
    const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_sesion_crear_desde_asignacion`, {
      method: 'POST', headers: this.headersJson(),
      body: JSON.stringify({ p_asignacion_id: dto.asignacion_id, p_actor_id: actor })
    });
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async checkInProponer(@Req() req, @Param('id') id: string, @Body() b: CheckInProponerDto) {
    const actor = (req.userId ?? req.user?.id) as string;
    const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_sesion_check_in_proponer_simple`, {
      method: 'POST', headers: this.headersJson(),
      body: JSON.stringify({ p_sesion_id: id, p_actor_id: actor, p_notas: b?.notas ?? null })
    });
    const txt = await r.text();
    if (!r.ok) throw new BadRequestException(txt || `HTTP ${r.status}`);
    return JSON.parse(txt) as Sesion;
  ...
  async checkInConfirmar(@Req() req, @Param('id') id: string) {
    const actor = (req.userId ?? req.user?.id) as string;
    const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_sesion_check_in_confirmar_simple`, {
      method: 'POST', headers: this.headersJson(),
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async checkOutProponer(@Req() req, @Param('id') id: string, @Body() b: CheckOutProponerDto) {
    const actor = (req.userId ?? req.user?.id) as string;
    const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_sesion_check_out_proponer_simple`, {
      method: 'POST', headers: this.headersJson(),
      body: JSON.stringify({ p_sesion_id: id, p_actor_id: actor, p_resumen: b?.resumen ?? null })
    });
    const txt = await r.text();
    if (!r.ok) throw new BadRequestException(txt || `HTTP ${r.status}`);
    return JSON.parse(txt) as Sesion;
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async checkOutConfirmar(@Req() req, @Param('id') id: string, @Body() b: CheckOutConfirmarDto) {
    const actor = (req.userId ?? req.user?.id) as string;
    const r = await fetch(`${this.baseUrl}/rest/v1/rpc/rpc_sesion_check_out_confirmar_simple`, {
      method: 'POST', headers: this.headersJson(),
      body: JSON.stringify({ p_sesion_id: id, p_actor_id: actor, p_resumen: b?.resumen ?? null })
    });
    const txt = await r.text();
  ...
  @Post(':id/resenas')
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async crearResena(@Req() req, @Param('id') sesionId: string, @Body() dto: CrearResenaDto) {
    const actor = (req.userId ?? req.user?.id) as string;

    // Resolver destinatario via join (cuidador vs solicitante)
    const joinUrl = `${this.baseUrl}/rest/v1/sesiones?` +
    ...
    if (dto.para === 'CUIDADOR') {
      if (s.solicitud?.usuario_id !== actor) throw new BadRequestException('Solo el dueño puede reseñar al cuidador');
      toUser = s.cuidador_id;
      rolFrom = 'SOLICITANTE';
    } else {
      if (s.cuidador_id !== actor) throw new BadRequestException('Solo el cuidador puede reseñar al usuario');
  ...
  // ───────────────────────────────
  @Get()
  async listMine(@Req() req, @Query('rol') rol: 'CUIDADOR' | 'SOLICITANTE' = 'CUIDADOR') {
    ...
    if (rol === 'CUIDADOR') {
      ...
      url = `${this.baseUrl}/rest/v1/sesiones?select=${encodeURIComponent(select)},` +
