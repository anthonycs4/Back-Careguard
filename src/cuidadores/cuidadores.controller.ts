...
export class CuidadoresController {
  ...
  async me(@Req() req) {
    ...
    const cuRows = await this.fetchJson<any[]>(cuUrl, { headers: this.headersJson() });
    ...
    const uRows = await this.fetchJson<any[]>(uUrl, { headers: this.headersJson() });
  ...
  async publicById(@Param('id') id: string) {
    const select = 'usuario_id,bio,anios_experiencia,horas_acumuladas,rating_promedio,tipos_servicio,tarifa_hora';
  ...
  async updateMe(@Req() req, @Body() dto: UpdateCuidadorDto) {
    ...
    const rows = await this.fetchJson<any[]>(existsUrl, { headers: this.headersJson() });
    if (!rows?.[0]) throw new BadRequestException('Aún no tienes perfil de cuidador');
    ...
    const res = await this.fetchJson<any[]>(patchUrl, {
      ...
      body: JSON.stringify({
        ...
        ...(dto.anios_experiencia !== undefined ? { anios_experiencia: dto.anios_experiencia } : {}),
        ...(dto.tarifa_hora !== undefined ? { tarifa_hora: dto.tarifa_hora } : {}),
      }),
    });
