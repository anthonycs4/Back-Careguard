...
async function supabasePasswordLogin(baseUrl: string, anonKey: string, email: string, password: string) {
  ...
  if (!res.ok) throw new BadRequestException(json?.error_description || json?.error || `HTTP ${res.status}`);
...
export class AuthController {
  ...
  async register(@Body() dto: RegisterDto) {
   ...
   const restUrl = `${this.baseUrl}/rest/v1/usuarios`;
    ...
    const session = await supabasePasswordLogin(this.baseUrl, this.anonKey, email, password);
  ...
  async updateAccount(@Req() req, @Body() dto: UpdateAccountDto) {
    ...
    if (dto.email || dto.password || dto.name) {
      ...
      if (error) throw new BadRequestException(`Error actualizando en Auth: ${error.message}`);
    }
    ...
    if (Object.keys(patchPayload).length > 0) {
      const resProfile = await fetch(`${baseUrl}/rest/v1/usuarios?id=eq.${userId}`, {
        method: 'PATCH',
        headers: {
          apikey: serviceKey,
          Authorization: `Bearer ${serviceKey}`,
      ...
      const resGet = await fetch(`${baseUrl}/rest/v1/usuarios?id=eq.${userId}&select=*`, {
